...
候选分三组（组间是硬顺序，组内按分数；这些是分数调参担保不了的，必须靠结构）：

| 组 | 内容 | 为什么在这个位置 |
|---|---|---|
| 0 | **完全覆盖输入**的整句 / 整键词 | 你已经打完的读音优先 |
| 1 | **补全候选**（尾音节没打完，`zhongguor → 中国人`） | 「猜你还没打完」不该插到已打完的读音前面（输入 `jian` 时前排不能被 `jiang` 的词占掉） |
| 2 | **部分候选**（只覆盖前一段，`nihaoshijie → 你好`） | 修复入口，不干扰正常整句选词 |
